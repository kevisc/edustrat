...
EduStrat is a static, client-side application: all statistical computation runs in the browser in JavaScript, with no server-side component, so the tool can be served from any static host or run locally with any HTTP server. The codebase comprises 21 ES6 JavaScript modules (~10,400 lines) organised into data-loading, analysis, visualisation, export, and interface layers, with comprehensive documentation (methodology, data sources, and a variable codebook).

The data architecture reflects a key constraint: serving approximately 3.5 million student records to a browser. Rather than loading the full ~1.25 GB dataset, EduStrat pre-generates 513 country--year JSON files and loads only the requested subsets (typically 30--40 MB). A pipeline of R scripts extracts, chunks, and validates PISA microdata from the `learningtower` package [@wang2024] and---for the cycles where design-correct standard errors are provided---re-sources the raw OECD Public Use Files to add the 80 replicate weights. In accordance with the OECD Terms of Use, the repository redistributes no micro-level data: the hosted demonstration serves the generated chunks, and a fresh clone fetches them from the demonstration on demand.
